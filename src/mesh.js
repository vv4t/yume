...
export class mesh_buffer_t {
  ...
  push(vertices) {
    const mesh = this.mesh_alloc(vertices.length);
    this.put(mesh, vertices);
    return mesh;
  }
  
  mesh_alloc(num_vertices) {
  ...
  put(mesh, vertices) {
    if (mesh.offset + vertices.length > this.max_vertices)
      throw "too many vertices";
    
    gl.bufferSubData(
      gl.ARRAY_BUFFER,
      mesh.offset * vertex_t.BYTE_SIZE,
